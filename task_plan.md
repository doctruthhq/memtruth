...
| 291. OpenDataLoader Bench external metrics RED/MVP | pending | Add benchmark report fields for external parser-quality metrics such as NID, TEDS, MHS, and speed, then gate audit-grade promotion when these parser-quality thresholds fail | Java/Rust benchmark report tests, runtime benchmark smoke |
| 292. Rust-default smoke reconciliation | complete | Review package, benchmark, OCR seed, real-PDF, ONNX, TATR, and SLANeXT smokes now follow the Rust-sidecar default parser path; review packages align `trust-document.json` page hashes with exported PNG manifests | review/model/benchmark/real-PDF/seed/ONNX/TATR/SLANeXT smokes, targeted Java/Rust tests |
