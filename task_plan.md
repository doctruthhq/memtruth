...
| 285. OpenDataLoader XY-Cut++ Rust port RED/MVP | pending | Port the Apache-2.0 XY-Cut++ reading-order behavior into `runtime/doctruth-runtime` as a Rust-owned geometry module with attribution, covering cross-layout elements, adaptive cuts, narrow-outlier filtering, two-column layouts, and sidebars | Rust RED tests adapted from OpenDataLoader-style scenarios, cargo test |
...
| 287. OpenDataLoader tagged-structure preference RED/MVP | pending | Detect usable PDF structure tree/tag information before heuristic reading-order guessing, while warning when tag quality is poor or incomplete | Rust tagged-PDF fixture tests, parse trace assertions |
...
| 289. Reference-composition guardrails | pending | Add docs/tests proving Kreuzberg, Docling, MinerU, and OpenDataLoader roles are layered and do not create competing canonical outputs; `TrustDocument` remains the only truth contract | PRD guardrail checks, output contract tests |
...
| 291. OpenDataLoader Bench adapter contract RED/MVP | pending | Add an adapter that exports DocTruth Rust runtime output into OpenDataLoader Bench-style prediction artifacts and imports `evaluation.json` into DocTruth benchmark reports under `external_metrics` | Java/Rust RED tests using a tiny local fixture and a checked-in synthetic `evaluation.json`; no GPL/AGPL engine execution |
| 292. OpenDataLoader Bench external metrics gate RED/MVP | pending | Add report/verifier fields for NID, TEDS, MHS, and speed, then block audit-grade promotion when parser-quality thresholds fail | Java/Rust benchmark report tests, runtime benchmark smoke |
