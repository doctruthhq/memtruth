...
`doctruth-java-core-phase10-title-heading-full200/full200` is the latest
recorded Java-core run. It parsed 200/200 documents in `15111.002791` ms, with
a mean `75.555014` ms/doc, lazy model startup enabled, and no
...
Overall now slightly beats the historical overall baseline `0.745414`, but
acceptance is not complete: TEDS remains at `0.341325` versus target `0.496416`,
and MHS remains just below target `0.483837`. The next high-impact gaps are
TEDS/table structure, remaining heading hierarchy misses, OCR/image-only
content, and broader reading-order/text normalization.
