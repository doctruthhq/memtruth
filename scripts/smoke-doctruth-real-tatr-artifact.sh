...
if [ "${DOCTRUTH_REAL_TATR_SMOKE:-0}" != "1" ]; then
    echo "skipping real TATR artifact smoke; set DOCTRUTH_REAL_TATR_SMOKE=1 to download/run the public ONNX artifact"
    exit 0
fi
...
if command -v python3 >/dev/null 2>&1; then
    ...
    echo "python3 is required for real TATR artifact smoke" >&2
    exit 1
fi

REPO="Xenova/table-transformer-structure-recognition"
VARIANT="${DOCTRUTH_REAL_TATR_VARIANT:-onnx/model_quantized.onnx}"
CACHE="${DOCTRUTH_REAL_TATR_CACHE:-target/real-tatr-cache}"
case "$CACHE" in
    /*) ;;
    *) CACHE="$ROOT/$CACHE" ;;
esac
WORK_DIR="${DOCTRUTH_REAL_TATR_SMOKE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/doctruth-real-tatr.XXXXXX")}"
MODEL="$CACHE/$(basename "$VARIANT")"
MANIFEST="$WORK_DIR/tatr-manifest.json"
PDF="$WORK_DIR/tatr-table-input.pdf"
REQUEST="$WORK_DIR/tatr-worker-request.json"
WORKER_OUT="$WORK_DIR/tatr-worker-output.json"
...
lines = [
    ...
    "72 432 m 72 648 l S",
    "228 432 m 228 648 l S",
    "384 432 m 384 648 l S",
    "540 432 m 540 648 l S",
    "BT /F1 16 Tf 96 615 Td (Name) Tj ET",
    "BT /F1 16 Tf 252 615 Td (Role) Tj ET",
    "BT /F1 16 Tf 408 615 Td (Score) Tj ET",
    "BT /F1 14 Tf 96 543 Td (Ada) Tj ET",
    "BT /F1 14 Tf 252 543 Td (Engineer) Tj ET",
    "BT /F1 14 Tf 408 543 Td (95) Tj ET",
    "BT /F1 14 Tf 96 471 Td (Lin) Tj ET",
    "BT /F1 14 Tf 252 471 Td (Analyst) Tj ET",
    "BT /F1 14 Tf 408 471 Td (88) Tj ET",
]
stream = "\n".join(lines) + "\n"
objects = [
...
cells = table["cells"]
assert cells, table
assert max(cell["rowRange"]["start"] for cell in cells) >= 1, cells
assert max(cell["columnRange"]["start"] for cell in cells) >= 1, cells
assert all(cell["boundingBox"]["x0"] < cell["boundingBox"]["x1"] for cell in cells), cells
assert all(cell["boundingBox"]["y0"] < cell["boundingBox"]["y1"] for cell in cells), cells
PY

DOCTRUTH_REAL_MODEL_MANIFEST="$MANIFEST" \
DOCTRUTH_REAL_MODEL_PRESET="table-lite" \
DOCTRUTH_REAL_MODEL_EXPECTED_ID="xenova-table-transformer-structure-recognition:$(basename "$VARIANT" .onnx)" \
...
echo "doctruth real TATR artifact smoke passed"
