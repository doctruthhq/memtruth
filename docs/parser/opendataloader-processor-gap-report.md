...
| List grouping | partial | `opendataloader_structure_contract` | full-bench list buckets pending | The structure probe groups sequential lower/upper letter lists, sequential numeric lists, and bullet lists, keeps non-sequential letter/numeric markers as paragraph text, and joins lowercase/connector continuation lines into the previous list item. Heading/caption classification takes priority over list grouping so numbered headings are not swallowed as single-item lists. Nested-list hierarchy and full-bench list evidence remain pending. |
