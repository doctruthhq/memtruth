...
| List grouping | partial | `opendataloader_structure_contract` | full-bench list buckets pending | The structure probe groups sequential lower/upper letter lists, sequential numeric lists, and bullet lists, and keeps non-sequential letter/numeric markers as paragraph text. Heading/caption classification takes priority over list grouping so numbered headings are not swallowed as single-item lists. Nested-list and wrapped-list continuation parity remains pending. |
