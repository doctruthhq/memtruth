...
| Borderless table clustering | partial | `PdfBorderlessTableExtractionTest` | current-full200 table buckets | Borderless heuristic covers aligned/ragged text tables plus long-header numeric rows; real image-only/model table cases and full cluster parity remain incomplete. |
