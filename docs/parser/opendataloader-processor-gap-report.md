...
`doctruth-java-core-phase8-sparse-grid-guard-full200/full200` is the latest
recorded Java-core run. It parsed 200/200 documents in `15235.8335` ms, with a
mean `76.179168` ms/doc, lazy model startup enabled, and no
...
The sparse-grid guard fixed a real class of table false positives, especially
content pages where one large text cell was being rendered as a fake table.
It did not materially change TEDS or MHS. The next high-impact gaps remain
heading hierarchy, markdown heading structure, OCR/image-only content, and
