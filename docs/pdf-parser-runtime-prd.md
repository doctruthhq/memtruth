...
Rust PDF substrate:
  ...
  page geometry, rendering, and bbox evidence. lopdf is transitional
  low-level/debug support only.
...
lopdf still owns:
  transitional table/debug content-stream extraction
...
| --- | --- | --- | --- | --- |
| Kreuzberg | Rust parser/runtime as the product core | Partial | `runtime/doctruth-runtime` has `parse_pdf`, `benchmark_corpus`, model-worker handoff, packaged sidecar, Java CLI sidecar wiring, OCR runtime-first selection, path-first SDK backend selection, and `pdf_oxide` text-layer extraction, page geometry, rendered image hashes, and bbox-backed line units | Rust runtime must become the default parser core, Java must become wrapper/oracle only, and remaining `lopdf` duties must be reduced to explicit table/debug compatibility support |
| Kreuzberg | `pdf_oxide`-style Rust PDF backend | Partial | Current Rust runtime uses `pdf_oxide` for text-layer page extraction, span bbox evidence, column-order post-processing, page MediaBox geometry, and default rendered PNG page hashes, and declares `parserRun.pdfBackend`; `lopdf` still provides table/debug extraction | Move table/debug extraction behind the Rust PDF backend boundary; keep `lopdf` only where it is explicitly needed as low-level/debug support |
...
| Docling | Parser backend/pipeline separation | Complete for contract, partial for Rust ownership | Parser presets, explicit Java oracle mode, sidecar backend, local worker protocols, Rust runtime commands, SDK backend modes | Java/PDFBox must not be a primary parser core; legacy document-first extraction remains compatibility only until it can call the Rust runtime by default |
...
Current implementation gap: `doctruth-runtime` now uses `pdf_oxide` for
text-layer page extraction, text span bbox evidence, and DocTruth-owned
column-order post-processing, page MediaBox geometry, and default rendered PNG
page hashes, and reports `parserRun.pdfBackend`. `lopdf` still owns table/debug
extraction. That mixed stack is useful as an incremental migration boundary,
but not the long-term core shape. The next Rust parser milestone is to move
table/debug extraction behind the Rust PDF backend boundary without going
through Java/PDFBox.
